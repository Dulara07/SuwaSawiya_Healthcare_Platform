...
interface Patient {
  ...
  mrn: string;
  dob: string;
  status: 'active' | 'stable' | 'critical' | 'pending' | 'discharged';
  lastVisit: string;
  doctor: string;
  ward: string;
}
...
export function PatientTable() {
  return <div className="bg-white border border-[#E0E0E0] rounded-[2px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] overflow-hidden">
      <div className="px-4 py-3 border-b border-[#E0E0E0] flex justify-between items-center bg-white">
        ...
        <button className="text-xs font-medium text-[#0066CC] hover:underline">
          View All Patients
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#F8F9FA] border-b border-[#E0E0E0]">
              <th className="px-4 py-3 text-xs font-bold text-gray-600 uppercase tracking-wider w-1/4">
                Patient Name
              </th>
              ...
              <th className="px-4 py-3 text-xs font-bold text-gray-600 uppercase tracking-wider text-right">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#E0E0E0]">
            {patients.map((patient, index) => <tr key={patient.id} className={`hover:bg-[#F0F7FF] transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-[#FAFAFA]'}`}>
                ...
                <td className="px-4 py-2.5">
                  <StatusBadge status={patient.status} type={patient.status} />
                </td>
                ...
                <td className="px-4 py-2.5 text-right">
                  <button className="text-gray-400 hover:text-[#0066CC] p-1 rounded-[2px] hover:bg-[#E6F6EC]">
