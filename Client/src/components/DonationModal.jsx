  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        <div className="flex justify-between items-center p-4 border-b border-gray-100">
          <h3 className="font-bold text-lg text-gray-900">Donate to {campaign.patientName}</h3>
          ...
          {step === 1 ? <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {presetAmounts.map(val => <button key={val} onClick={() => setAmount(val.toString())} className={`py-2 px-4 rounded-lg border text-sm font-medium transition-all ${amount === val.toString() ? 'border-blue-600 bg-blue-50 text-blue-700 ring-1 ring-blue-600' : 'border-gray-200 hover:border-blue-300 text-gray-700'}`}>{val.toLocaleString()}</button>)}
                </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg flex items-start space-x-3">
                ...
                <p className="text-xs text-blue-800 leading-relaxed">Your donation is protected. 100% of funds go directly to the verified hospital account for {campaign.patientName}.</p>
              </div>
              <Button fullWidth size="lg" disabled={!amount} onClick={() => setStep(2)}>Continue</Button>
              ...
              <div className="pt-4">
                <Button fullWidth size="lg" isLoading={isLoading} onClick={handleDonate} className="flex items-center justify-center"><Lock className="w-4 h-4 mr-2" />Pay {campaign.currency} {Number(amount).toLocaleString()}</Button>
                <button onClick={() => setStep(1)} className="w-full mt-3 text-sm text-gray-500 hover:text-gray-700">Back to amount</button>
              </div>
            </div>}
        </div>
