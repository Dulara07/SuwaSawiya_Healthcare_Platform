  ...
  return <header className="bg-white shadow-sm sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center md:space-x-8">
            ...
            {user && <Link to="/profile" className={`text-sm font-medium ${isActive('/profile') ? 'text-blue-600' : 'text-gray-500 hover:text-gray-900'}`}>{t('nav.profile')}</Link>}
            ...
            {user?.role === 'admin' && <Link to="/admin/dashboard" className={`text-sm font-medium ${isActive('/admin/dashboard') ? 'text-blue-600' : 'text-gray-500 hover:text-gray-900'}`}>{t('nav.adminDashboard')}</Link>}
          </div>
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            ...
            {user ? <div className="flex items-center space-x-3 ml-4 border-l pl-4 border-gray-200">
                ...
                <button onClick={logout} className="text-gray-400 hover:text-gray-600" title={t('nav.logout')} aria-label={t('nav.logout')}><LogOut className="h-5 w-5" /></button>
              </div> : <div className="flex items-center space-x-2 ml-4">
                <Button variant="ghost" size="sm" onClick={() => navigate('/login/donor')}>{t('nav.donorLogin')}</Button>
                <Button variant="outline" size="sm" onClick={() => navigate('/login/partner')}>{t('nav.partnerLogin')}</Button>
                <Button variant="primary" size="sm" onClick={() => navigate('/login/admin')}>{t('nav.adminLogin')}</Button>
              </div>}
          </div>
      ...
      {isMenuOpen && <div className="md:hidden bg-white border-b border-gray-200">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {user && <Link to="/profile" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>{t('nav.profile')}</Link>}
            ...
            {user?.role === 'admin' && <Link to="/admin/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-blue-600 bg-blue-50" onClick={() => setIsMenuOpen(false)}>{t('nav.adminDashboard')}</Link>}
            {!user && <div className="pt-4 pb-2 border-t border-gray-100 mt-2">
                <div className="space-y-2 px-3">
                  <Button fullWidth variant="outline" onClick={() => { navigate('/login/donor'); setIsMenuOpen(false); }}>{t('nav.donorLogin')}</Button>
                  <Button fullWidth variant="outline" onClick={() => { navigate('/login/partner'); setIsMenuOpen(false); }}>{t('nav.partnerLogin')}</Button>
                  <Button fullWidth variant="primary" onClick={() => { navigate('/login/admin'); setIsMenuOpen(false); }}>{t('nav.adminLogin')}</Button>
                </div>
              </div>}
            {user && <div className="pt-4 pb-2 border-t border-gray-100 mt-2 px-3">
                ...
                <Button fullWidth variant="outline" onClick={() => { logout(); setIsMenuOpen(false); }}>{t('nav.logout')}</Button>
              </div>}
          </div>
        </div>}
    </header>;
